function H = HeightEstimator(img,mask_ground,mask_target,mask_person1,mask_person2,ref_height1,ref_height2,point_target,lines)
    ...
    [H,W,~] = size(img);
    hold on
    drawLines(lines, W, 12)
    [vps, vps_class, vph] = get_vps(lines, 12, H, W);
    scatter(point_target(1), point_target(2), 'x')
    p1_xy = drawPersonLine(mask_person1);
    p2_xy = drawPersonLine(mask_person2);
    t_xy = drawTargetLine(mask_ground, point_target(1:2));
    ...
    vpv1 = estimateVpv(p1_line, t_line);
    vpv2 = estimateVpv(p2_line, t_line);

    [p1_t, p1_b] = selectTopBottom(p1_xy);
    [p2_t, p2_b] = selectTopBottom(p2_xy);
    [t_t, t_b] = selectTopBottom(t_xy);

    H1 = estimateHeight(vpv1, vph, p1_t, p1_b, ref_height1, t_t, t_b);
    H2 = estimateHeight(vpv2, vph, p2_t, p2_b, ref_height2, t_t, t_b);
    H = (H1 + H2)/2
    ...
    function drawLines(lines, W, max_lines_nums)
        k = -lines(:, 1)./lines(:, 2);
        b = -lines(:, 3)./lines(:, 2);
        c_index = kmeans(k, 2);
        for i=1:min(length(lines), max_lines_nums)
    ...
    function person_xy = drawPersonLine(person_mask)
    ...
    function target_xy = drawTargetLine(ground_mask, target_point)
        d_min = Inf;
        d_min_point = [0, 0];
    ...
    function vpv = estimateVpv(line1, line2)
        vpv = cross(line1, line2);
        vpv = vpv / vpv(3);
        scatter(vpv(1), vpv(2), 'p')
    end

    function [top, bottom] = selectTopBottom(xy)
        if(xy(2, 1) > xy(2, 2))
            top = xy(:, 2);
            bottom = xy(:, 1);
        else
    ...
    function target_height = estimateHeight(vpoint_vertical, vline, ref_top, ref_bottom, ref_height, target_top, target_bottom)
