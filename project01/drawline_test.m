...
[vps, vps_class, vph] = get_vps(lines, max_lines_nums, H, W);
...
function [vps, vps_class, vph] = get_vps(lines, max_lines_nums, H, W)
    ...
    farthest_dis = zeros(1, length(vps(1, :)));
    nearest_dis = zeros(1, length(vps(1, :)));
    for i=1:length(farthest_dis)
         farthest_d = -Inf;
         nearest_d = Inf;
        for j=1:length(farthest_dis)
            if(i == j)
                continue;
            end
            if((vps(1, i)-vps(1, j))^2 + (vps(2, i)-vps(2, j))^2 > farthest_d)
                farthest_d = (vps(1, i)-vps(1, j))^2 + (vps(2, i)-vps(2, j))^2;
            end
            if((vps(1, i)-vps(1, j))^2 + (vps(2, i)-vps(2, j))^2 < nearest_d)
                nearest_d = (vps(1, i)-vps(1, j))^2 + (vps(2, i)-vps(2, j))^2;
            end
        end
        farthest_dis(i)= farthest_d;
        nearest_dis(i)= nearest_d;
    end
    
    dis = 1 ./ (nearest_dis).^1.02;
    
    partial_k_k = sum( dis.*vps(1,:).^2 );
    partial_k_b = sum( dis.*vps(1,:) );
    partial_k_c = sum( dis.*vps(1,:).*vps(2,:) );
    partial_b_k = partial_k_b;
    partial_b_b = sum( dis );
    partial_b_c = sum( dis.*vps(2,:) );
